...
from app.forms import CommentForm,SubscribeForm,UserForm
...
def index(request):
    ...
    if request.POST:
        ...
        if subscribe_form.is_valid():
            subscribe_form.save()
            subscribe_successful= 'Subscribed Successfuly'
            subscribe_form=SubscribeForm()
...
def post_page(request, slug):
    ...
    if request.POST:
        ...
        if comment_form.is_valid:
            parent_obj=None
            if request.POST.get('parent'):#save reply
                ...
                if parent_obj:
                 ...
                 comment = comment_form.save(commit=False)#form will not be saved but we can add values in it.
